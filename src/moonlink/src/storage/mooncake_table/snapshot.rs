...
impl SnapshotTableState {
    ...
    // Update current snapshot's file indices by adding and removing a few, generated by index merge or data compaction.
    async fn update_file_indices_to_mooncake_snapshot_impl(
        ...
        println!("process old file indices {}", old_file_indices.len());
        ...
        for cur_file_index in old_file_indices.iter() {
            ...
            for cur_data_file in cur_file_index.files.iter() {
                    ...
                    self.current_snapshot.disk_files.get_mut(cur_data_file)
                {
                    ...
                    println!("what ??? disk file dosn't exist in currentsnapshot");
                }
            }
        }
    ...
    async fn integrate_disk_slices(&mut self, task: &mut SnapshotTask) -> Vec<String> {
        // Aggregate evicted cache files to delete.
        let mut evicted_files_to_delete = vec![];
