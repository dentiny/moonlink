...
async fn test_streaming_begin_flush_commit_end_flush() {
    ...
    // Apply the flush
    table.apply_stream_flush_result(xact_id, disk_slice, uuid::Uuid::new_v4());
...
async fn test_streaming_begin_flush_commit_end_flush_multiple() {
    ...
    // Apply the flush
    table.apply_stream_flush_result(xact_id, disk_slice1, uuid::Uuid::new_v4());
    ...
    table.apply_stream_flush_result(xact_id, disk_slice2, uuid::Uuid::new_v4());
...
async fn test_streaming_begin_flush_delete_commit_end_flush() {
    ...
    table.apply_stream_flush_result(xact_id, disk_slice, uuid::Uuid::new_v4());
    table.apply_stream_flush_result(xact_id2, disk_slice2, uuid::Uuid::new_v4());
...
async fn test_ongoing_flush_lsns_tracking() -> Result<()> {
    ...
    table.apply_flush_result(disk_slice_2, uuid::Uuid::new_v4());
    assert!(table.ongoing_flush_lsns.contains(&5));
    assert!(!table.ongoing_flush_lsns.contains(&10));
    assert!(table.ongoing_flush_lsns.contains(&15));
    ...
    table.apply_flush_result(disk_slice_1, uuid::Uuid::new_v4());
    assert!(!table.ongoing_flush_lsns.contains(&5));
    assert!(table.ongoing_flush_lsns.contains(&15));
    assert_eq!(table.get_min_ongoing_flush_lsn(), 15); // Now 15

    // Complete last flush
    table.apply_flush_result(disk_slice_3, uuid::Uuid::new_v4());
    assert!(table.ongoing_flush_lsns.is_empty());
    assert_eq!(table.get_min_ongoing_flush_lsn(), u64::MAX);
...
async fn test_streaming_flush_lsns_tracking() -> Result<()> {
    ...
    table.apply_stream_flush_result(xact_id_1, disk_slice_1, uuid::Uuid::new_v4());
    table.apply_stream_flush_result(xact_id_2, disk_slice_2, uuid::Uuid::new_v4());
    table.apply_flush_result(disk_slice_3, uuid::Uuid::new_v4());
...
async fn test_out_of_order_flush_completion() -> Result<()> {
    ...
    table.apply_flush_result(disk_slice_30, uuid::Uuid::new_v4());
    assert!(!table.ongoing_flush_lsns.contains(&30));
    assert!(table.ongoing_flush_lsns.contains(&10));
    assert!(table.ongoing_flush_lsns.contains(&20));
    ...
    table.apply_flush_result(disk_slice_10, uuid::Uuid::new_v4());
    assert!(!table.ongoing_flush_lsns.contains(&10));
    assert!(table.ongoing_flush_lsns.contains(&20));
    assert_eq!(table.get_min_ongoing_flush_lsn(), 20);

    // Complete flush 20 (should clear all)
    table.apply_flush_result(disk_slice_20, uuid::Uuid::new_v4());
    assert!(table.ongoing_flush_lsns.is_empty());
    assert_eq!(table.get_min_ongoing_flush_lsn(), u64::MAX);
...
async fn test_mixed_regular_and_streaming_lsn_ordering() -> Result<()> {
    ...
    table.apply_stream_flush_result(xact_id, streaming_disk_slice, uuid::Uuid::new_v4());
    assert!(!table.ongoing_flush_lsns.contains(&50));
    assert!(table.ongoing_flush_lsns.contains(&100));
    ...
    table.apply_flush_result(regular_disk_slice, uuid::Uuid::new_v4());
    assert!(table.ongoing_flush_lsns.is_empty());
    assert_eq!(table.get_min_ongoing_flush_lsn(), u64::MAX);
...
async fn test_streaming_empty_batch_filtering() -> Result<()> {
    ...
    table.apply_stream_flush_result(xact_id1, disk_slice1, uuid::Uuid::new_v4());
