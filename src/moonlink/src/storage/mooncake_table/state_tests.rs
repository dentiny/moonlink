...
/// - remote, no local, in use + use over => remote, no local, not use
///
...
async fn prepare_test_disk_file(
    ...
    return (table, table_notify);
}

/// Test util function to import a second data file cache entry.
async fn import_fake_cache_entry(cache: &mut ObjectStorageCache) {
    ...
    cache.import_cache_entry(FAKE_FILE_ID, cache_entry).await;
}

/// Test scenario: no remote, local, no use + use => no remote, local, in use
#[tokio::test]
...
/// Test scenario: remote, local, in use + use over & unpinned => remote, local, not used
#[tokio::test]
...
/// Test scenario: remote, no local, in use + use & unpinned => remote, local, in use
#[tokio::test]
async fn test_2_read_and_unpinned_3() {
    let temp_dir = tempfile::tempdir().unwrap();
    let cache_config = ObjectStorageCacheConfig::new(
        ONE_FILE_CACHE_SIZE,
        temp_dir.path().to_str().unwrap().to_string(),
