        ...
        loop {
            tokio::select! {
                ...
                Some(event) = table_notify_rx.recv() => {
                    match event {
                        TableNotify::MooncakeTableSnapshot { lsn, iceberg_snapshot_payload, data_compaction_payload, file_indice_merge_payload, evicted_data_file_cache } => {
                            // Spawn a detached best-effort task to delete evicted data file cache.
                            if !evicted_data_file_cache.is_empty() {
